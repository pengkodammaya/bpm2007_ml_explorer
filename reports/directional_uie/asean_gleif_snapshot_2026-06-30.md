...
This snapshot reflects the completed Singapore reporting-exception refresh. The regional directional aggregates are unchanged from the 2026-06-26 snapshot after the Singapore refresh, but the evidence base is now cleaner because SG reporting exceptions have been re-run to completion. Timor-Leste is included in the regional coverage set but has no inward or outward rows in the current directional result pack.
...
## Singapore Refresh Note

The Singapore reporting-exception refresh completed with 10,697 exception-bearing entities out of 10,875 scanned entities. The reason distribution was:

| Exception reason | Count |
|---|---:|
| NON_CONSOLIDATING | 4,020 |
| NATURAL_PERSONS | 2,905 |
| NO_KNOWN_PERSON | 1,584 |
| NO_LEI | 1,265 |
| NON_PUBLIC | 918 |
| CONSENT_NOT_OBTAINED | 5 |

After rerunning SG scoring and regenerating the ASEAN directional pack, the headline directional aggregates remained unchanged. This indicates the refreshed exceptions strengthened the evidence base, especially for SG entity flags, but did not materially change the current country-level UIE assignment totals.
...
- `reports/directional_uie/asean_inward_summary_2026-06-30.csv`
- `reports/directional_uie/asean_inward_density_2026-06-30.csv`
- `reports/directional_uie/asean_outward_summary_2026-06-30.csv`
- `reports/directional_uie/asean_outward_source_density_2026-06-30.csv`
