...
class PipelineTests(unittest.TestCase):
    ...
    def test_save_uie_assignment_snapshot_uses_country_and_utc_timestamp(self) -> None:
        import src.pipeline as pipeline

        with tempfile.TemporaryDirectory() as tmp:
            original_reports_dir = pipeline.REPORTS_DIR
            pipeline.REPORTS_DIR = Path(tmp)
            try:
                df = pd.DataFrame([{"lei": "D1", "uie_country": "US"}])
                path = save_uie_assignment_snapshot(
                    df,
                    "my",
                    pd.Timestamp("2026-06-26T00:30:00Z").to_pydatetime(),
                )
            finally:
                pipeline.REPORTS_DIR = original_reports_dir

            expected = (
                Path(tmp)
                / "uie_snapshots"
                / "my"
                / "uie_assignments_MY_20260626T003000Z.csv"
            )
            self.assertEqual(path, expected)
            self.assertTrue(expected.exists())
            saved = pd.read_csv(expected)
            self.assertEqual(saved.to_dict("records"), [{"lei": "D1", "uie_country": "US"}])
